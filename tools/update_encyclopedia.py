...
<div class="encySearchWrap">
  <span>🔎</span><input id="encySearch" type="search" placeholder="知りたいことを検索…" oninput="filterEncyclopedia()">
</div>
<div class="encyQuick" id="encyQuick">
  <button onclick="filterEncyclopedia('水')">💧 水</button>
  <button onclick="filterEncyclopedia('トイレ')">🚽 トイレ</button>
  <button onclick="filterEncyclopedia('停電')">🔦 停電</button>
  <button onclick="filterEncyclopedia('避難')">🎒 避難</button>
  <button onclick="filterEncyclopedia('連絡')">👨‍👩‍👧 連絡</button>
</div>
...
<script>
(function(){
  const list=document.getElementById('encyList');
  if(!list)return;
  window.openEncyclopediaItem=function(item){
    const was=item.classList.contains('open');
    document.querySelectorAll('.encyItem.open').forEach(x=>x.classList.remove('open'));
    if(!was)item.classList.add('open');
  };
  window.filterEncyclopedia=function(term){
    const input=document.getElementById('encySearch');
    if(term!==undefined) input.value=term;
    const q=(input.value||'').trim().toLowerCase();
    let shown=0;
    list.querySelectorAll('.encyItem').forEach(item=>{
      const hit=!q || (item.dataset.keywords+' '+item.innerText).toLowerCase().includes(q);
      item.style.display=hit?'flex':'none';
      if(hit)shown++;
    });
    document.getElementById('encyCount').textContent=shown+'件の備え';
    document.getElementById('encyEmpty').hidden=shown!==0;
  };
})();
</script>
<style>
.encyHero{background:linear-gradient(135deg,#dff4d9,#fff3b8);border-radius:24px;padding:20px;text-align:center;box-shadow:0 5px 18px rgba(50,90,50,.08)}
.encyBook{font-size:42px;line-height:1}.encyHero .pageTitle{margin:5px 0}.encyHero p{margin:0;font-size:13px;line-height:1.7}
.encySearchWrap{display:flex;align-items:center;gap:8px;margin:16px 0 9px;background:#fff;border-radius:17px;padding:12px 14px;box-shadow:0 3px 12px rgba(50,90,50,.07)}
.encySearchWrap input{width:100%;border:0;outline:0;font:inherit;background:transparent;color:#36563b}.encySearchWrap input::placeholder{color:#94a994}
.encyQuick{display:flex;gap:7px;overflow-x:auto;padding:2px 1px 9px;scrollbar-width:none}.encyQuick::-webkit-scrollbar{display:none}.encyQuick button{white-space:nowrap;background:#fff;border-radius:99px;padding:8px 12px;color:#4b7651;box-shadow:0 2px 8px rgba(50,90,50,.06);font-size:12px}
.encyCount{font-size:12px;color:#78907a;margin:4px 2px 8px}.encyList{display:flex;flex-direction:column;gap:10px}.encyItem{position:relative;display:flex;align-items:flex-start;gap:13px;background:#fff;border-radius:20px;padding:16px 42px 16px 14px;box-shadow:0 3px 12px rgba(50,90,50,.07);cursor:pointer;flex-wrap:wrap}.encyIcon{width:48px;height:48px;display:flex;align-items:center;justify-content:center;flex:0 0 48px;background:#edf7e9;border-radius:15px;font-size:28px}.encyItem>div:nth-child(2){flex:1;min-width:0}.encyItem h3{margin:0 0 5px;color:#397a44;font-size:17px}.encyItem p{margin:0;font-size:12px;line-height:1.65;color:#527056}.encyArrow{position:absolute;right:17px;top:31px;font-size:28px;color:#4e9956}.encyDetail{display:none!important;flex-basis:100%;padding:13px 4px 2px;border-top:1px solid #e6efe2;margin-top:3px}.encyItem.open .encyDetail{display:block!important}.encyItem.open .encyArrow{transform:rotate(90deg)}.encyTip{margin-top:10px;padding:10px 12px;background:#f2f9ed;border-radius:13px;font-size:12px;line-height:1.6}.encyEmpty{margin-top:14px;background:#fff;border-radius:18px;padding:22px;text-align:center;color:#66806a;line-height:1.8}.encyPage>.smallNote{margin-top:15px;text-align:center}
</style>
</section>'''
