(function(){
  ...
  async function uploadPostMedia(file){
    ...
    if(file.size>max)throw new Error(isVideo?'動画は50MB以下にしてね。':'写真は8MB以下にしてね。');
  ...
  function installPostMedia(){
    ...
    note.textContent=registered()?'写真8MB・動画50MBまで。':'ゲスト投稿は文章のみ。写真・動画は登録後に使えます。';if(!registered())note.classList.add('mediaGuestNote');
